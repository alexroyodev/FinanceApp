...
export default function InvestmentsView({ user, allAssets = [], totalInvested = 0, onDataChange }: InvestmentsViewProps) {
  const { getToken } = useAuth();
  
  // Estados para Nuevo Activo
  const [newAssetName, setNewAssetName] = useState('');
  const [newAssetType, setNewAssetType] = useState('Fondo Indexado');
  const [newAssetBalance, setNewAssetBalance] = useState('');
  const [newAssetAccountId, setNewAssetAccountId] = useState('');
  
  // Estados para Aportaciones
  ...
  const handleAddAsset = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { 
        setNewAssetName(''); setNewAssetType('Fondo Indexado'); setNewAssetBalance(''); 
  ...
  // Prevenimos renderizar si el usuario aún no cargó
  if (!user) return <div className="text-gray-500">Cargando datos...</div>;
  ...
  return (
    <div className="grid md:grid-cols-2 gap-8 animate-fade-in transition-colors">
      <div className="space-y-8">
          ...
          <form onSubmit={handleAddAsset} className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow border border-gray-100 dark:border-gray-700 space-y-4 border-l-4 border-blue-500 transition-colors">
            <div className="flex gap-4">
              ...
              <div className="flex-1">
                ...
                <select value={newAssetType} onChange={(e) => setNewAssetType(e.target.value)} className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg p-2 outline-none focus:ring-2 focus:ring-blue-500">
                  <option value="Fondo Indexado">Fondo Indexado</option>
              ...
              <>
                ...
                <div>
                  ...
                  <select value={contributionOriginAccountId} onChange={(e) => setContributionOriginAccountId(e.target.value)} className="w-full bg-white dark:bg-gray-700 border border-gray-300 dark:border-gray-600 text-gray-900 dark:text-white rounded-lg p-2 outline-none focus:ring-2 focus:ring-blue-500">
                    {user?.accounts?.map((account: any) => <option key={account.id} value={account.id}>{account.name} (Disp: {Number(account.balance || 0).toFixed(2)}€)</option>)}
                  </select>
                </div>
        ...
        <div>
          <h3 className="text-xl font-bold text-gray-800 dark:text-white mb-4">Registrar Rendimiento</h3>
      ...
      <div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-100 dark:border-gray-700 overflow-hidden sticky top-8 transition-colors">
          <div className="bg-blue-50 dark:bg-blue-900/30 p-4 border-b border-blue-100 dark:border-blue-800 flex justify-between items-center transition-colors">
            ...
            <span className="text-2xl font-black text-blue-700 dark:text-blue-400">{Number(totalInvested || 0).toFixed(2)} €</span>
          </div>
          {allAssets?.length === 0 ? (
                ...
                <div key={asset.id} className="p-5 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors flex justify-between items-center">
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      {asset.symbol && (
                        <span className="bg-blue-100 dark:bg-blue-900/50 text-blue-700 dark:text-blue-300 text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wider">
                          {asset.symbol}
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-4">
                    <p className="text-xl font-bold text-gray-900 dark:text-white">{Number(asset.balance || 0).toFixed(2)} €</p>
